...
public class RPG implements Gun{
	...
	public void shoot(Player player) {
		...
		player.getWorld().playSound(player.getLocation(), Sound.ENTITY_WITHER_SHOOT,
				8, 2);
		player.getWorld().playSound(player.getLocation(), Sound.ENTITY_ENDERDRAGON_FIREBALL_EXPLODE,
				8, 0.7f);		
		}catch(Error e2){
			player.getWorld().playSound(player.getLocation(), Sound.valueOf("CLICK"), 5,
					1);
			player.getWorld().playSound(player.getLocation(), Sound.valueOf("WITHER_SHOOT"),
					8, 2);
			player.getWorld().playSound(player.getLocation(), Sound.valueOf("EXPLODE"),
					8, 0.7f);		}
		}
	...
	public void reload(Player player) {
		GunUtil.basicReload(this, player,GunType.isUnlimited(GunType.RPG),2.5);		
	}
...
public boolean hasUnlimitedAmmo() {
	return GunType.isUnlimited(GunType.RPG);
}
