...
public class BulletProtectionUtil {
	...
	public static boolean stoppedBullet(Player p, Location bullet, Vector velocity) {
		if (p.getInventory().getHelmet() != null) {
			if (me.zombie_striker.qualityarmory.api.QualityArmory.isArmor(p.getInventory().getHelmet())) {
				ArmorObject armor = me.zombie_striker.qualityarmory.api.QualityArmory.getArmor(p.getInventory().getHelmet());
