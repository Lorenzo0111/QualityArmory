...
public class GunnerTrait extends Trait {
    ...
    @Override
    public void load(final DataKey key) { this.SomeSetting = key.getBoolean("SomeSetting", false); }

    // Save settings for this NPC (optional). These values will be persisted to the
    // Citizens saves file
    @Override
    public void save(final DataKey key) { key.setBoolean("SomeSetting", this.SomeSetting); }

    // An example event handler. All traits will be registered automatically as
    // Bukkit Listeners.
    @EventHandler
    public void click(final net.citizensnpcs.api.event.NPCClickEvent event) {
    ...
    @Override
    public void onAttach() { this.plugin.getServer().getLogger().info(this.npc.getName() + "has been assigned MyTrait!"); }
